...
from src.trail.models import Trail, DayPlan, OnsenNode


def generate_markdown(trail: Trail, all_nodes: list[OnsenNode] | None = None) -> str:
    ...
    transit_days = sum(1 for d in trail.days if d.walking_km > 0 and not d.onsens_visited)
    onsen_days = sum(1 for d in trail.days if d.onsens_visited)
    rest_days = 65 - trail.total_days  # Assuming 65-day window
    ...
    lines.append(f"| Walking days | {walking_days} ({onsen_days} with onsens, {transit_days} transit) |")
    ...
    lines.append(f"| Prefectures | {len(pref_counts)} ({', '.join(f'{p} ({c})' for p, c in pref_counts.most_common())}) |")
    ...
    for day in trail.days:
        ...
        lines.append(f"### Day {day.day_number} — {day.date.strftime('%b %d')} ({weekday})")
        lines.append("")
        lines.append(f"**Distance:** {day.walking_km:.1f} km | **Onsens:** {len(day.onsens_visited)}")
        ...
        if day.onsens_visited:
            ...
            for i, onsen in enumerate(day.onsens_visited):
                ...
                dist_str = f"{day.segments[i].distance_km:.1f} km" if i < len(day.segments) else "—"
                lines.append(
    ...
    for day in trail.days:
        ...
        for i, onsen in enumerate(day.onsens_visited):
            ...
            day_data["onsens"].append({
                ...
                "distance_km": round(day.segments[i].distance_km, 1) if i < len(day.segments) else None,
            })
...
def generate_csv(trail: Trail) -> str:
    ...
    writer.writerow([
        "day", "date", "weekday", "km", "onsens_count",
        "onsen_names", "overnight_type", "warnings_count",
    ])
...
def save_outputs(
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        f.write(generate_markdown(trail, all_nodes))
