...
# Base URL template for onsen detail pages
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                ...
                logger.warning(
                    f"Attempt {attempt}/{max_retries} failed for onsen {onsen_id}: {e}. "
                    f"Retrying in {wait}s..."
                )
                time.sleep(wait)
            else:
                logger.error(
                    f"All {max_retries} attempts failed for onsen {onsen_id}: {e}"
                )
