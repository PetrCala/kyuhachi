 ...
 *
 * Photos live at visits/{userId}/{onsenId}/photo.jpg, where onsenId is the
 * visit doc ID (the onsen's kyuhachiId). Deleting the visit doc — whether a
 * single-visit delete or as part of deleting a whole challenge — would
 * otherwise leak the photo in Storage forever.
 *
...
export const onVisitDeleted = onDocumentDeleted(
  ...
  async (event) => {
    const { userId, onsenId } = event.params;

    const prefix = `visits/${userId}/${onsenId}/`;
