...
const ActiveChallengeContext = createContext<ActiveChallengeProgress>({
  loading: true,
  hasChallenge: null,
  challengeId: null,
  challenge: null,
...
/**
 * The single active-challenge subscription for the whole app: the user →
 * challenge → visits chain, the challenge type (tiers / completion target /
 * baseMode), the eligible onsens' display data, and the active route. The
 * claimed tier (`challenge.earnedTier`) is read straight off the challenge doc;
 * eligibility for the next claim is derived here and the claim is committed
 * through the `claimTier` server callable.
 *
 * Provider-scoped rather than per-screen on purpose. Every screen mounting its
 * own copy meant each one opened a fresh listener chain and painted once with
 * an empty `visitedIds` before the visits snapshot landed. The onsen list
 * showed that as a visible re-order (rows jumping from the unvisited block to
 * the visited one, section counts stepping up) a frame or two after the tab
 * opened. Subscribing once at the root means the state is already resolved
 * before any tab mounts, so a screen's first paint is its final one. It also
 * shares one set of listeners with the splash gate (see `useBootReady`), which
 * already waits on this data before the first frame.
 */
export function ActiveChallengeProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<ActiveChallengeProgress>(
    () => ({
      loading,
      hasChallenge,
      challengeId,
  ...
  return <ActiveChallengeContext.Provider value={value}>{children}</ActiveChallengeContext.Provider>;
}

export function useActiveChallengeProgress(): ActiveChallengeProgress {
  return useContext(ActiveChallengeContext);
}
